...
def parse(file_name: str) -> dict:
    ...
    try:
        ...
        e.throw(e.FAIL, e.UNICODE_DECODE_ERROR, f"'utf-8' codec can't decode byte")
    except PermissionError:
        e.throw(e.FAIL, e.PERMISSION_ERROR, f"permission denied: {file_name}")
    except json.decoder.JSONDecodeError:
        e.throw(e.FAIL, e.JSON_DECODE_ERROR, "JSON decode error")
...
def __general_check(json_file: dict):
    ...
    if list(json_file.keys()) != primary_keys:
        e.throw(e.FAIL, e.JSON_FORMAT_ERROR, f"primary keys has to be: {', '.join(primary_keys)}")
    elif __is_empty(json_file.values()):
        e.throw(e.FAIL, e.EMPTY_FIELD_ERROR, "primary values can't be empty")
...
def __alphabet_check(json_file: dict):
    ...
    if __is_not_list(values):
        e.throw(e.FAIL, e.TYPE_ERROR, "alphabet has to be contained in a list")
    elif __is_empty(values):
        e.throw(e.FAIL, e.EMPTY_FIELD_ERROR, "alphabet can't be empty")
    elif __is_not_string(values):
        e.throw(e.FAIL, e.TYPE_ERROR, "alphabet values have to be string")
    elif __is_repeated(values):
        e.throw(e.FAIL, e.REPETITION_ERROR, "alphabet values have to be unique")
    elif __is_not_size_one(values):
        e.throw(e.FAIL, e.REPETITION_ERROR, "alphabet values have to be size 1")


def __name_check(json_file: dict):
    if __is_not_string([json_file['name']]):
        e.throw(e.FAIL, e.TYPE_ERROR, "name has to be a string")
...
def __blank_check(json_file: dict):
    ...
    if __is_not_string([value]):
        e.throw(e.FAIL, e.TYPE_ERROR, "blank character has to be a string")
    elif __is_not_in([value], json_file['alphabet']):
        e.throw(e.FAIL, e.JSON_FORMAT_ERROR, f"blank character '{value}' should be part of the alphabet")
...
def __states_check(json_file: dict):
    ...
    if __is_not_list(values):
        e.throw(e.FAIL, e.TYPE_ERROR, "states have to be contained in a list")
    elif __is_empty(values):
        e.throw(e.FAIL, e.EMPTY_FIELD_ERROR, "states can't be empty")
    elif __is_not_string(values):
        e.throw(e.FAIL, e.TYPE_ERROR, "states values have to be string")
    elif __is_repeated(values):
        e.throw(e.FAIL, e.REPETITION_ERROR, "states values have to be unique")
...
def __initial_check(json_file: dict):
    ...
    if __is_not_string([value]):
        e.throw(e.FAIL, e.TYPE_ERROR, "initial state has to be a string")
    elif __is_not_in([value], json_file['states']):
        e.throw(e.FAIL, e.JSON_FORMAT_ERROR, f"initial state '{value}' should be one of states")
...
def __finals_check(json_file: dict):
    ...
    if __is_not_list(values):
        e.throw(e.FAIL, e.TYPE_ERROR, "final states have to be contained in a list")
    elif __is_not_string(values):
        e.throw(e.FAIL, e.TYPE_ERROR, "final states have to be string")
    elif __is_repeated(values):
        e.throw(e.FAIL, e.REPETITION_ERROR, "final states have to be unique")
    elif __is_not_in(values, json_file['states']):
        e.throw(e.FAIL, e.JSON_FORMAT_ERROR, f"final states '{', '.join(values)}' have to be in states")
...
def __transitions_check(json_file: dict):
    ...
    if __is_not_dict(values):
        e.throw(e.FAIL, e.TYPE_ERROR, "transitions have to be contained in a dict")
    elif __is_not_in(values.keys(), json_file['states']):
        e.throw(e.FAIL, e.JSON_FORMAT_ERROR, f"transitions '{', '.join(values.keys())}' have to be in states")
    for transition_state in values.values():
        if __is_not_list(transition_state):
            e.throw(e.FAIL, e.TYPE_ERROR, "transitions states description have to be contained in a list")
        elif __is_empty([transition_state]):
            e.throw(e.FAIL, e.EMPTY_FIELD_ERROR, "transitions states description can't be empty")
        for transition in transition_state:
            if list(transition.keys()) != transition_keys:
                e.throw(e.FAIL, e.JSON_FORMAT_ERROR,
